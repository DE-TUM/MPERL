...
from src.models import PonderRelationalGraphConvModel, RelationalGraphConvModel, ReconstructionLoss, RegularizationLoss
...
from src.losses import edl_mse_loss, edl_digamma_loss, edl_digamma_loss_no_markov, edl_log_loss, relu_evidence
...
class Train:
    def __init__(self, args):
        ...
        self.loss_rec_no_markov = edl_digamma_loss_no_markov
    ...
    def train(self, epoch):  
        ...
        if self.args.markov == 'yes':
            ...
            y = one_hot_embedding(self.labels[self.idx_train], self.y.shape[1])
            loss_rec_no_markov = self.loss_rec_no_markov(emb_train[self.idx_train], y.float(), epoch, self.y.shape[1], 10*y.shape[1])
        ...
        if self.args.using_cuda:
            self.labels = self.labels.cuda()
        if self.args.loss_function == 'erl':
            if self.args.markov == 'yes':
                # Case 2: mperl, evidential regularization loss
                loss = loss_rec + self.beta * loss_reg
            elif self.args.markov == 'no':
                # Case 1: no mperl, evidnetial but no regularization loss 
                loss = loss_rec_no_markov
        elif self.args.loss_function == 'edl':
            if self.args.markov == 'yes':
                #Case 5: mperl, evidential loss with halting state took into account 
                loss = loss_rec 
            elif self.args.markov == 'no':
                # Case 1: no mperl, evidnetial but no regularization loss 
                loss = loss_rec_no_markov
            ...
            if self.args.markov == 'yes':
                # Case 4: mperl, binary cross entropy loss
                loss = loss_bce(emb_train[self.idx_train], self.labels[self.idx_train])  
            elif self.args.markov == 'no':  
                #Case 3: no mperl, binary cross entropy loss
                loss = loss_bce(emb_train[self.idx_train], self.labels[self.idx_train])
        ...
        if self.args.markov == 'yes':
            ...
            if self.args.loss_function == 'erl':
                ...
                loss_test = self.loss_rec(emb_train[self.idx_test], y.float(), epoch, self.y.shape[1], 10*y.shape[1], torch.mean(tmp), 0)
            elif self.args.loss_function == 'bce':
                loss_test = loss_bce(emb_train[self.idx_test], self.labels[self.idx_test])
        elif self.args.markov == 'no':
            if self.args.loss_function in ('erl', 'edl'):
                loss_test = self.loss_rec_no_markov(emb_train[self.idx_test], y.float(), epoch, self.y.shape[1], 10*y.shape[1]) 
            elif self.args.loss_function == 'bce':
                loss_test = loss_bce(emb_train[self.idx_test], self.labels[self.idx_test])    
    ...
    def test(self):
        with torch.no_grad():
            ...
            if self.args.markov == 'yes':
                ...
                y = one_hot_embedding(self.labels[self.idx_test], self.y.shape[1])
                loss_rec_no_markov = self.loss_rec_no_markov(emb_test[self.idx_test], y.float(), epoch, self.y.shape[1], 10*y.shape[1])
            ...
            if self.args.loss_function == 'erl':
                ...
                if self.args.markov == 'yes':
                    # Case 4: mperl, binary cross entropy loss
                    loss_test = loss_bce(emb_test[self.idx_test], self.labels[self.idx_test])  
                elif self.args.markov == 'no':  
                    #Case 3: no mperl, binary cross entropy loss
                    loss_test = loss_bce(emb_test[self.idx_test], self.labels[self.idx_test])
